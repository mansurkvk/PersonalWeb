      ...
      <div className="relative mx-auto grid max-w-7xl gap-6 xl:grid-cols-[300px_1fr]">
        <aside className="glass-panel lab-border h-fit rounded-[2.2rem] p-5 shadow-[0_28px_80px_rgba(0,0,0,0.32)]">
          ...
          <div className="mt-5 rounded-[1.4rem] border border-white/10 bg-black/20 p-4">
            <div className="flex items-center justify-between"><span className="text-xs uppercase tracking-[0.2em] text-slate-500">MongoDB</span><span className={`text-xs font-semibold ${fetchState === "error" ? "text-[#f2a47d]" : "text-[#9bd0b8]"}`}>{fetchState === "syncing" ? "syncing" : fetchState === "error" ? "retrying" : "connected"}</span></div>
            <div className="mt-3 h-2 overflow-hidden rounded-full bg-white/[0.06]"><div className={`h-full rounded-full bg-gradient-to-r from-[#8bd3dd] to-[#d5b46a] transition-all duration-700 ${fetchState === "error" ? "w-1/3" : fetchState === "empty" ? "w-1/2" : "w-full"}`} /></div>
          </div>
        ...
        <main className="space-y-5">
          <section className="glass-panel lab-border relative overflow-hidden rounded-[2.6rem] p-6 sm:p-8">
            ...
            <div className="mt-8 grid gap-3 md:grid-cols-4"><HeroStat label="Device" value={latest?.deviceId ?? "No device"} icon={Cpu} /><HeroStat label="Packets" value={String(packetCount)} icon={Activity} /><HeroStat label="Refresh" value={`${iotConfig.refreshMs / 1000}s`} icon={RefreshCw} /><HeroStat label="Last seen" value={formatRelativeTime(lastSeen)} icon={Clock} /></div>
          </section>
