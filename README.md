...
PersonalWeb, Mansur Kavak icin gelistirilen Next.js tabanli bir portfolio, engineering lab, robotik urun vitrini ve IoT telemetry platformudur. Proje; kisisel siteyi, teknik icerikleri, ESP32 veri akisini, admin panelini, urun sayfalarini ve iletisim surecini tek modular monolith mimaride birlestirir.
...
- Admin paneli ve admin mesaj yonetimi
...
`src/app/` icinde business logic tutulmaz. Route handler request parse eder, session/guard okur, service fonksiyonunu cagirir ve response uretir.
...
Admin ve normal kullanici tek login sayfasi olan `/login` uzerinden giris yapar. Admin rolune sahip kullanici `/admin` sayfasina, normal kullanici `/profile` sayfasina yonlendirilir.
...
Oturum varsa kullanicinin e-postasi session uzerinden okunur ve formda bilgi olarak gosterilir. Oturum yoksa e-posta alani zorunlu input olarak gosterilir.
...
`/api/contact` rate-limit katmanini kullanir ve kullanici girdilerini service katmaninda validate eder.
...
- Gercek servis anahtarlari veya sifreler kaynak kodda birakilmaz.
